...
export default function Prestataires() {
  ...
  const [form, setForm] = useState({ nom: "", contact: "", telephone: "", email: "", contrat_reference: "", cout_horaire: "" });

  const load = async () => { const { data } = await supabase.from("prestataires").select("*").order("nom"); setItems(data || []); };
  useEffect(() => { load(); }, []);

  const openNew = () => { setEditing(null); setForm({ nom: "", contact: "", telephone: "", email: "", contrat_reference: "", cout_horaire: "" }); setOpen(true); };
  const openEdit = (p: any) => { setEditing(p); setForm({ nom: p.nom, contact: p.contact || "", telephone: p.telephone || "", email: p.email || "", contrat_reference: p.contrat_reference || "", cout_horaire: p.cout_horaire ? String(p.cout_horaire) : "" }); setOpen(true); };

  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { nom: form.nom, contact: form.contact || null, telephone: form.telephone || null, email: form.email || null, contrat_reference: form.contrat_reference || null, cout_horaire: form.cout_horaire ? Number(form.cout_horaire) : null };
    ...
    toast.success(editing ? "Mis à jour" : "Créé"); setOpen(false); load();
  };
        ...
        <Card>
          <table className="w-full text-sm data-table">
            <thead><tr className="border-b border-border">
              <th className="px-4 py-3 text-left">Nom</th><th className="px-4 py-3 text-left">Contact</th>
              <th className="px-4 py-3 text-left">Téléphone</th><th className="px-4 py-3 text-left">Email</th>
              <th className="px-4 py-3 text-left">Contrat</th><th className="px-4 py-3 text-right">Coût/h</th>
              {canEdit && <th className="px-4 py-3 text-right">Actions</th>}
            </tr></thead>
            <tbody>
              {items.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-muted-foreground"><Briefcase className="mx-auto mb-2 h-8 w-8 opacity-40" />Aucun prestataire</td></tr>
              : items.map((p) => (
