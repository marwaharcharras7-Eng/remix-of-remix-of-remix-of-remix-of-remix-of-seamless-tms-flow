...
export default function Chauffeurs() {
  ...
  const [form, setForm] = useState({
    ...
    disponibilite: true, consommation_moyenne: "30", flotte_id: "none",
  });
  ...
  const openNew = () => { setEditing(null); setForm({ nom: "", prenom: "", numero_permis: "", type_permis: "C", telephone: "", disponibilite: true, consommation_moyenne: "30", flotte_id: "none" }); setOpen(true); };
  const openEdit = (c: any) => {
    ...
    setForm({
      ...
      consommation_moyenne: String(c.consommation_moyenne), flotte_id: c.flotte_id || "none",
    });
    setOpen(true);
  };
      ...
      <div className="flex-1 p-6">
        <Card>
          <table className="w-full text-sm data-table">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={8} className="py-12 text-center text-muted-foreground"><Users className="mx-auto mb-2 h-8 w-8 opacity-40" />Aucun chauffeur</td></tr>
              ) : items.map((c) => (
                <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/40">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums">{Number(c.taux_performance).toFixed(0)}%</td>
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={save} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5"><Label>Conso moy. (L/100km)</Label><Input type="number" step="0.1" value={form.consommation_moyenne} onChange={(e) => setForm({ ...form, consommation_moyenne: e.target.value })} /></div>
            </div>
